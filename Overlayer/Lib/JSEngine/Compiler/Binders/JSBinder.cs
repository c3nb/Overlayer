...
namespace JSEngine.Compiler
{
    ...
    internal class JSBinder : MethodBinder
    {
        ...
        private static void EmitTypeConversion(ILGenerator il, Type fromType, Type toType)
        {
            ...
            {
                PrimitiveTypeUtilities.ToPrimitiveType(fromType, out var pType);
                EmitConversion.ToInteger(il, pType);
            }
            else if (typeof(ObjectInstance).IsAssignableFrom(toType))
            {
                PrimitiveTypeUtilities.ToPrimitiveType(fromType, out var pType);
                EmitConversion.Convert(il, pType, PrimitiveType.Object);
            ...
            {
                PrimitiveTypeUtilities.ToPrimitiveType(fromType, out var from);
                PrimitiveTypeUtilities.ToPrimitiveType(toType, out var to);
                EmitConversion.Convert(il, from, to);
            }
        }
