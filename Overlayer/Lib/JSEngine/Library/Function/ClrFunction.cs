...
using Binder = JSEngine.Compiler.Binder;
...
namespace JSEngine.Library
{
    ...
    public class ClrFunction : FunctionInstance
    {
        ...
        /// <param name="length"> The "typical" number of arguments expected by the function.  Pass
        /// <c>-1</c> to use the number of arguments expected by the delegate. </param>
        internal ClrFunction(ObjectInstance prototype, MethodInfo methodToCall, string name = null, int length = -1)
            : base(prototype)
        {
            // Initialize the [[Call]] method.
            this.callBinder = new JSBinder(new JSBinderMethod(methodToCall));
        ...
        private class EmptyBinder : Binder
        {
            ...
            /// <returns> A delegate that does type conversion and calls the method represented by this
            /// object. </returns>
            protected override void GenerateStub(JSEngine.Compiler.ILGenerator generator, int argumentCount)
            {
                // Emit undefined.
