...
{
    public class CompilableJavaScriptImpl : Impl
    {
        public override ScriptType ScriptType => ScriptType.CompilableJS;
        public override string Generate()
        {
            ...
            foreach (var (attr, api) in Api.GetApiMethodsWithAttr(ScriptType))
            {
                ...
                if (options.Length > 0)
                {
                    var opt = options.Select(p => (p.ParameterType, p.Name)).Where(t => t.ParameterType != typeof(ScriptEngine)).ToArray();
        ...
        private static string GetTypeHintCode(Type type)
        {
            ...
            if (api?.Name != null) return api.Name;
            if (type == typeof(void))
                return "void";
            else if (typeof(Array).IsAssignableFrom(type) ||
                      typeof(ArrayInstance) == type)
                return "any[]";
