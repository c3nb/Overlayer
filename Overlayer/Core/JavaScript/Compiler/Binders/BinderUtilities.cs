...
using ErrorType = Overlayer.Core.JavaScript.Library.ErrorType;
        ...
        public static int ResolveOverloads(RuntimeMethodHandle[] methodHandles, ScriptEngine engine, object thisValue, object[] arguments)
        {
            ...
            for (int i = 0; i < methods.Length; i++)
            {
                foreach (var argument in methods[i].GetArguments(arguments.Length))
                {
                    ...
                    if (input is Overlayer.Core.JavaScript.Library.ClrInstanceWrapper)
                        input = ((Overlayer.Core.JavaScript.Library.ClrInstanceWrapper)input).WrappedInstance;
